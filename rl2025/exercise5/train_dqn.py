...
ENV = "MOUNTAINCAR" # "CARTPOLE" is also possible if you uncomment the corresponding code, but is not assessed for DQNModified.
# ENV = "CARTPOLE"
...
elif MOUNTAINCAR_CONFIG['epsilon_decay_strategy'] == "constant":
    MOUNTAINCAR_HPARAMS = MOUNTAINCAR_HPARAMS_CONSTANTS
else:
    MOUNTAINCAR_HPARAMS = None

SWEEP_RESULTS_FILE_MOUNTAINCAR = f"DQNModified-MountainCar-sweep-decay-{MOUNTAINCAR_CONFIG['epsilon_decay_strategy']}-results.pkl"
...
def train(env: gym.Env, config, output: bool = True) -> Tuple[np.ndarray, np.ndarray, np.ndarray, Dict]:
    """     
    Execute training of DQNModified on given environment using the provided configuration
    ...
    :return (Tuple[np.ndarray, np.ndarray, np.ndarray, Dict]): average eval returns during training, evaluation
            timesteps, compute times at evaluation and a dictionary containing other training metrics specific to DQNModified
    """
    timesteps_elapsed = 0
    ...
    with tqdm(total=config["max_timesteps"]) as pbar:
        while timesteps_elapsed < config["max_timesteps"]:
            ...
            if timesteps_elapsed % config["eval_freq"] < episode_timesteps:
                eval_returns = 0
                if config["env"] == "CartPole-v0" or config["env"] == "CartPole-v1" or config["env"] == "MountainCar-v0":
                    max_steps = config["episode_length"]
                else:
